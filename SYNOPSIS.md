...
2. **Reinforcement Learning from Human Feedback (RLHF)** — A self-improving data loop bridging human expertise with AI. When an NGO Admin corrects an AI prediction on the dashboard, the system automatically logs the correction. Offline, the BERT model is re-trained on these corrections using Supervised Fine-Tuning. This allows the system to continuously learn and align with real-world veterinary triage standards without new code.
